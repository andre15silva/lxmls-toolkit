...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        query = query.view(B, T, self.num_heads, self.hidden_size // self.num_heads).transpose(1, 2)
        key = key.view(B, T, self.num_heads, self.hidden_size // self.num_heads).transpose(1, 2)
        value = value.view(B, T, self.num_heads, self.hidden_size // self.num_heads).transpose(1, 2)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        keys = [k for k in keys if not re.match(r"transformer\.h\.\d+\.attn\.bias", k)]  # ignore these
        sd_keys = [k for k in sd if not re.match(r"transformer\.h\.\d+\.attn\.bias", k)]  # ignore these
